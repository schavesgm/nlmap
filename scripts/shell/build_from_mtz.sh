...
OUT_LOG=output.log
BUC_LOG=$(basename ${PROT_PATH})_refmtz.log

# -- Save the current path, move to directory and save new directory
BASE_PATH=$(pwd); cd ${OUT_PATH}; END_PATH=$(pwd)

# -- Assert we are in the correct directory
assert_location ${BASE_PATH} ${OUT_PATH} ${END_PATH}

# -- Remove all files in the current directory
rm -r ./*

# -- Generate the output for buccaneer
cad HKLIN1 ${PROT_PATH}/dataset.mtz HKLIN2 ${PROT_PATH}/refmac.mtz HKLOUT buccaneer-input.mtz > ${OUT_LOG} <<eof
